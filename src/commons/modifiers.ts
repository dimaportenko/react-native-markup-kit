...
export function extractColorValue(props: Dictionary<any>) {
  ...
  const colorPropsKeys = _.chain(props)
    ...
    .value();
  const color = _.findLast(
    colorPropsKeys,
    (colorKey) => props[colorKey] === true
  )!;
  return Colors[color];
}
...
export function extractBackgroundColorValue(props: Dictionary<any>) {
  ...
  const bgProp = _.findLast(
    keys,
    (prop) => Colors.getBackgroundKeysPattern().test(prop) && !!props[prop]
  )!;
  if (props[bgProp]) {
...
export function extractTypographyValue(
  ...
  _.forEach(paddingPropsKeys, (key) => {
    if (props[key] === true) {
      ...
      if (!isNaN(Number(paddingValue))) {
        ...
        paddings[paddingVariation] =
          Spacings[paddingValue as keyof typeof SpacingLiterals];
      }
    }
  });
...
export function extractMarginValues(props: Dictionary<any>) {
  ...
  _.forEach(marginPropsKeys, (key) => {
    if (props[key] === true) {
      const [marginKey, marginValue] = key.split('-') as [
        keyof typeof MARGIN_VARIATIONS,
        string
      ];
      ...
      if (!isNaN(Number(marginValue))) {
        ...
        margins[paddingVariation] =
          Spacings[marginValue as keyof typeof SpacingLiterals];
      }
    }
  });
...
export function extractPositionStyle(props: Dictionary<any>) {
  ...
  const keys = Object.keys(props);
  const positionProp = _.findLast(
    keys,
    (prop) => POSITION_KEY_PATTERN.test(prop) && !!props[prop]
  );
  if (positionProp) {
    const positionVariationKey = _.split(
      positionProp,
      'abs'
    )[1] as keyof typeof POSITION_CONVERSIONS;
...
export function extractFlexStyle(
  props: Dictionary<any>
): Partial<Record<NativeFlexModifierKeyType, number>> | undefined {
  const keys = Object.keys(props);
  const flexProp = keys.find((item) => FLEX_KEY_PATTERN.test(item));
  if (flexProp && props[flexProp] === true) {
    let [flexKey, flexValue] = flexProp.split('-') as [
      keyof typeof STYLE_KEY_CONVERTERS,
      string
    ];
...
export function extractBorderRadiusValue(props: Dictionary<any>) {
  ...
  const keys = Object.keys(props);
  const radiusProp = keys.find(
    (prop) => BorderRadiuses.getKeysPattern().test(prop) && props[prop]
  ) as BorderRadiusLiterals;
  if (radiusProp) {
...
export function getThemeProps(props = this.props, context = this.context) {
  const componentName = //@ts-ignore
    this.displayName || this.constructor.displayName || this.constructor.name;
...
export function getAlteredModifiersOptions(currentProps: any, nextProps: any) {
  ...
  const changedKeys = _.filter(
    allKeys,
    (key) => !_.isEqual(currentProps[key], nextProps[key])
  );
  ...
  if (
    _.find(changedKeys, (key) => Colors.getBackgroundKeysPattern().test(key))
  ) {
